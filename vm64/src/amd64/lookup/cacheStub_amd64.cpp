...
# include "_cacheStub_amd64.cpp.incl"

# if defined(FAST_COMPILER) || defined(SIC_COMPILER)

// PIC (polymorphic inline cache) code emission for amd64
// (cf. cacheStub_aarch64.cpp).
//
// Entered through a send site's target word: receiver at
// [rsp + leaf_rcvr_offset*oopSize], [rsp] = the sendDesc return PC (and
// must stay intact for the miss path).  Map constants and case targets
// are pooled absolute words; the pool is flushed at the end of finish(),
// so the loc sequence is (one word per immediate case, then map/jump
// pairs, then the fail target) exactly as getJumpLocsIndex/
// getMapLocsIndex expect.  Tag scheme: Int=0, Mem=1, Float=2.
//
// Temp1 holds the receiver (then its map); Temp2 is the tag/compare/jump
// scratch, playing the role x16/x17 play on aarch64.

Label* CacheStub::br_if_not_smi() {
  Label* not_smi = new Label(a->printing);
  a->tst(Temp1, Tag_Mask);
  a->jcc(x64_ne, not_smi);
  return not_smi;
}
...
Label* CacheStub::prologue(bool immediateOnly) {
  ...
  if (immediateOnly)
    ...
    a->jcc(x64_e, loadMapAfterHandlingImmediates);
  }
  ...
  if (loadMapAfterHandlingImmediates) {
    ...
    a->load(Temp1, Temp1, map_offset());   // tagged-pointer offset
  }
  return miss;
}
...
Label* CacheStub::test(oop map, pc_t addr, Label* prev) {
  if (prev)
    prev->define();
  a->loadOopLiteral(Temp2, map);   // pool entry order: map before its jump
  a->cmp(Temp1, Temp2);
  Label* next_test = new Label(a->printing);
  a->jcc(x64_ne, next_test);
  jump(addr);
  return next_test;
}
...
void CacheStub::finish(Label* miss, Label* prev) {
  ...
  if (miss)
    miss->define();
  a->loadAddressLiteral(Temp2, (void*)theSendDesc->lookupRoutine(),
                        PVMAddressOperand);
  a->jmp_reg(Temp2);               // [rsp] still holds the sendDesc's retPC
