...
int read_wrap(int fd, char *buf, int buf_len, 
  ...
  // With the GUI console (macOS Finder launch), a BLOCKING read of stdin
  // -- the console's pty -- must pump events while waiting: the data it
  // wants can only arrive via the console's keyDown, which needs the very
  // pump this read would starve.  The ^C interrupt menu reads stdin this
  // way (preemptReadLine, after suspend_and_resume cleared O_NONBLOCK);
  // without this it deadlocks showing the beachball.  Nonblocking reads
  // keep their exact semantics.  No-op off macOS / without the console.
  { extern bool VMConsoleActive;
    extern void vm_console_block_until_input();
    if (VMConsoleActive && fd == 0 && !(fcntl(fd, F_GETFL, 0) & O_NONBLOCK))
